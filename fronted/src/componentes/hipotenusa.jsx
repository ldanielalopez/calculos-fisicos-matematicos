...
function Hipotenusa() {
    ...
    const [catetoA, SetCatetoA] = useState(undefined);
    const [catetoB, SetCatetoB] = useState(undefined);
    const [resultado, SetResultado] = useState(null);

    const calcularHipotenusa = async() => {
        const respuesta = await 
        fetch ("http://localhost:3000/matematicas/hipotenusa", {          // indicamos la ruta que vamos a consumir
    ...
    return (
        <div className="bg-white p-10 rounded-2xl shadow-xl border border-slate-100 w-full max-w-xl mx-auto mt-10">
            <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Calcular Hipotenusa</h2>
            ...
            <div className="space-y-6">
                ...
                <div>
                    <input
                        type="number"
                        placeholder="Ingrese el valor del Cateto B"
                        className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-600 focus:border-cyan-600 transition-all duration-200"
